...
# Author: Erik Westrup <erik.westrup@gmail.com>, Jean Jouve <jean-18-09@hotmail.fr>
# Dependencies:
...
mailboxes=(main/Inbox ens/INBOX)			# Mailboxes to watch.
email_parse="$HOME/.local/bin/email_parse.py"
...
parse_send() {
	...
	local subject=$($email_parse --subject "$mailfile")
	local from=$($email_parse --from "$mailfile")
	local body=$($email_parse --body "$mailfile")
	...
	# Send the message 
	notify-send "$out_summary" "$out_body"
}
