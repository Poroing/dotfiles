...
battery_level () {
    level=$(acpi -b | awk '/Battery 0/ { print $4 }')
    echo -n ${level%"%,"}
}

battery_state () {
    state=$(acpi -b | awk '/Battery 0/ { print $3 }')
    state=${state%,}
    echo -n $state
}
...
/home/jean/.config/personal/alert.sh -b "$background_color" -f "$foreground_color" "$battery_alert_text"
